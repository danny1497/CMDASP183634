...
int main (int argc, char **argv) {
  ...
  // Public key consists of (p,g,h) must broadcast each number
  ...
  if (rank ==1) {
    ...
    MPI_Send(message, Nmessages, MPI_UNSIGNED, 0, 2, MPI_COMM_WORLD);
    MPI_Send(a, Nmessages, MPI_UNSIGNED, 0, 3, MPI_COMM_WORLD); 
  }
  ...
  if (rank == 0) {
    ...
    MPI_Recv(message,Nmessages, MPI_UNSIGNED, 1, 2, MPI_COMM_WORLD, status);
    MPI_Recv(a, Nmessages, MPI_UNSIGNED, 1, 3, MPI_COMM_WORLD, status);
